...
2. Use `npm run commit` to keep each logical change in a single Conventional Commit that `git-cliff` can summarize cleanly.
3. Run `npm run changelog:unreleased` during iteration to preview only the pending changelog entries without flooding agent context.
4. Run `npm run changelog` when you are ready to update the full file, then curate the generated output into Keep a Changelog sections so the first stable release tells the story of the template, not just raw commit subjects.
5. Keep the `Unreleased` section at the top and preserve the `<!-- generated by git-cliff -->` marker so future updates stay automatable.
